...
public class EmailJob {
	 ...
	 */
	public EmailJob(ProjectService projectService, CandidateService candidateService, TaskService taskService,
			JavaMailConfig javaMailBuilder, ReleaseService releaserService, EmailService emailService) {
	 ...
	 * <p>
	 * Retrieves sender and subject, constructs subject regarding task, retrieves
	 * recipients and constructs content.
	 * 
	...
	public Mail buildMail(String taskName) {
		...
		String content = javaMailBuilder.buildMail(getArgs(candidates), template);
	...
	private String getSubject(String taskName) {
		...
		StringBuffer subject = new StringBuffer("[SPRINTPLANNER] ").append(firstPart).append(secondPart)
				.append(releaseVersion);
		return subject.toString();
	}

	private List<String> getRecipients(String taskName, Map<String, CandidateViewDto> candidates) {
	...
	private Map<String, Object> getArgs(Map<String, CandidateViewDto> candidates) {
	...
	private String buildFullName(CandidateViewDto candidate) {
		...
		StringBuffer sb = new StringBuffer(member.getFirstname()).append(" ").append(member.getLastname());
		return sb.toString();
	...
	private Map<String, CandidateViewDto> getCandidates(String taskName) {
		...
		if (taskName.equals(Constants.RELEASER)) {
			CandidateViewDto releaser = candidateService.findFirstByTaskNameAndStatusAndMemberShift(Constants.RELEASER,
					Status.CURRENT, null);
			CandidateViewDto tester = candidateService.findFirstByTaskNameAndStatusAndMemberShift(Constants.TESTER,
					Status.CURRENT, null);
			...
			Arrays.asList(Shift.PAR, Shift.BGL)
					.forEach(shift -> Arrays.asList(Constants.FUNCTIONAL, Constants.TECHNICAL).forEach(task -> {
						final CandidateViewDto candidate = candidateService
								.findFirstByTaskNameAndStatusAndMemberShift(task, Status.CURRENT, shift);
						candidates.put(task + shift.name(), candidate);
					}));
		} else {
			throw new NotYetImplementedException("given task (" + taskName + ") is not yet handled...");
		}
		return candidates;
	}
}
